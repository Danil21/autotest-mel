...
public class CommentTest extends SetDriver {
    ...
    public void addAndDeleteComment() {
        ...
        autoLogin.authorisationInPageReact(config.getTestProperty("userLogin"), config.getTestProperty("userPass"));
        ...
        $(comment.commentHeader).shouldBe(Condition.text("Комментарии(2)")); //
        sleep(1000);
        Assert.assertTrue(comment.showComments());
        comment.deleteComment();
        // check "no comment text"
        Assert.assertTrue(comment.checkIsDisplayedCommentHeader());
        $(comment.commentHeader).shouldBe(Condition.text("Комментарии(1)"));
        ...
        sleep(1000);
        Assert.assertEquals(methods.getTextFromSelector(comment.textComment), "Я не смогла дочитать дальше середины. Такой «кудрявый» текст читать просто невозможно: " +
                "в одном предложении столько информации упаковано, что поневоле останавливаешься, чтобы его обдумать, прежде чем читать дальше. Сюжет становится " +
                "при этом уже неинтересен. На какую аудиторию рассчитана эта книга? Она понравится людям вроде Анатолия Вассермана, обладающим академическими знаниями и" +
                " привыкшими изучать энциклопедии. Интересуюсь историей Иерусалима, обожаю читать и имею высшее гуманитарное образование, но эту книгу я не куплю.");
        ...
        Assert.assertEquals(methods.getTextFromSelector(mailAuthorisation.textInComplaintReviewLetter), "Пользователь mail test пожаловался на комментарий пользователя" +
                " Ирина Зорина в публикации Как капитан Монти Паркер искал главную святыню иудеев и обхитрил всех.");
